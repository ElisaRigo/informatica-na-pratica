...
interface PixPaymentRequest {
  ...
  cpf: string;
  phone?: string;
  amount?: number; // Opcional: se não fornecido, usa o preço padrão
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Usar o valor fornecido ou pegar do ambiente
    const coursePrice = amount || parseFloat(Deno.env.get("COURSE_PRICE") || "297.00");
