...
import aprendaComigoThumb from "@/assets/aprenda-comigo-capa.jpg";
...
const Informatica = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ─── ENVIRONMENT / AULA REAL (Light) ─── */}
      <section className="py-8 md:py-12 bg-white relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10">
          <div ref={envRef} className="max-w-4xl mx-auto">
            <div className="text-center mb-4">
              ...
              <p className="text-muted-foreground text-sm md:text-base max-w-2xl mx-auto">
                Essa é uma das +90 aulas do curso completo. Assista e sinta a didática simples, passo a passo!
              </p>
            </div>

            {shouldLoadEnv && (
              <div className="relative rounded-xl md:rounded-2xl overflow-hidden shadow-2xl border-2 border-slate-200 mb-4">
                {!isEnvPlaying ? (
                  <div className="relative aspect-video cursor-pointer group" onClick={() => setIsEnvPlaying(true)}>
                    <img src={environmentThumb} alt="Aula gratuita de Excel" className="w-full h-full object-cover" loading="lazy" />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-14 h-14 md:w-20 md:h-20 rounded-full bg-white/70 shadow-xl flex items-center justify-center group-hover:scale-110 transition-all">
                    ...
                    <div className="absolute top-2 right-2 md:top-3 md:right-3 animate-pulse">
                      <div className="bg-red-500 text-white px-2 py-1 md:px-4 md:py-2 rounded-full font-bold text-[10px] md:text-sm border-2 border-white/30 shadow-lg">🎬 Aula Real</div>
                    </div>
                  </div>
                ) : (
                  <div className="aspect-video">
            ...
            <div className="text-center">
              <button onClick={() => (window as any).openCheckout?.()} className="group w-full max-w-xl mx-auto flex items-center justify-center gap-2 bg-primary/10 text-primary font-bold text-base md:text-lg px-6 md:px-8 py-3 md:py-4 rounded-xl border border-primary/30 hover:bg-primary/20 transition-all">
                <span>Bora começar essa jornada <span className="text-success font-bold">comigo?</span></span>
      ...
      <section className="py-8 md:py-12 bg-gradient-to-br from-primary to-accent relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-6">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-white mb-4">Imagine você daqui a 30 dias...</h2>
          ...
          <div className="max-w-4xl mx-auto space-y-3">
            {transformations.map((item, i) => (
              <div key={i} className="flex items-center justify-between gap-4 bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20 hover:bg-white/20 transition-all group">
                <div className="flex-1 text-right"><span className="text-white/60 line-through">{item.before}</span></div>
                ...
                <div className="flex-1"><span className="text-white font-bold">{item.after}</span></div>
              </div>
            ))}
          </div>
          <div className="text-center mt-6">
            <button onClick={() => (window as any).openCheckout?.()} className="inline-flex items-center justify-center gap-3 bg-white text-primary font-black text-lg md:text-xl px-10 py-4 rounded-full shadow-2xl hover:scale-105 transition-all">
      ...
      <section className="py-10 md:py-16 bg-slate-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <span className="inline-block bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-semibold mb-4">Histórias Reais</span>
            <h2 className="text-3xl md:text-5xl font-black text-foreground mb-4">
              Mais de <span className="text-primary">15.000 vidas</span> transformadas
            </h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Veja o que nossos alunos estão dizendo sobre o curso</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
            {testimonials.map((t, i) => (
              <div key={i} className="group bg-white rounded-2xl p-6 border border-slate-200 hover:border-primary/40 hover:shadow-xl transition-all hover:-translate-y-1">
                ...
                <div className="flex items-center gap-3">
                  <img src={t.image} alt={t.name} className="w-12 h-12 rounded-full object-cover border-2 border-primary/30" loading="lazy" />
                  <div>
          ...
          <div className="grid grid-cols-3 gap-4 max-w-3xl mx-auto mt-10">
            {[{ number: "15.000+", label: "Alunos" }, { number: "98%", label: "Satisfação" }, { number: "4.9", label: "Avaliação" }].map((s, i) => (
              <div key={i} className="text-center">
                <p className="text-3xl md:text-4xl font-black text-primary">{s.number}</p>
      ...
      {/* ─── PRICING ─── */}
      <section id="preco" className="py-10 md:py-16 bg-gradient-to-b from-white to-slate-100">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <span className="inline-block bg-amber-100 text-amber-700 px-4 py-2 rounded-full text-sm font-bold mb-4 animate-pulse">
          ...
          <div className="max-w-4xl mx-auto">
            <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-primary/20">
              <div className="bg-gradient-to-r from-primary to-accent py-3 px-6 text-center">
                <p className="text-white font-bold text-base md:text-lg">ECONOMIA DE MAIS DE 60%</p>
              </div>
              <div className="p-6 md:p-10">
                <div className="grid md:grid-cols-2 gap-8">
                  <div>
                    <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2"><Zap className="w-5 h-5 text-primary" />O que você recebe:</h3>
                    <ul className="space-y-2 mb-6">
                      {included.map((item, i) => (
                        <li key={i} className="flex items-center gap-3">
                          <div className="w-5 h-5 bg-accent/20 rounded-full flex items-center justify-center flex-shrink-0"><Check className="w-3 h-3 text-accent" /></div>
                          <span className="text-foreground text-sm">{item.text}</span>
                        </li>
                      ))}
                    </ul>
                    <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                      <h4 className="font-bold text-foreground mb-2 flex items-center gap-2 text-sm"><Gift className="w-4 h-4 text-amber-500" />BÔNUS EXCLUSIVOS:</h4>
                      <ul className="space-y-1.5">
                        {bonuses.map((b, i) => (
                          <li key={i} className="flex items-center justify-between text-xs">
                            <span className="text-muted-foreground">{b.text}</span>
                            <span className="text-destructive line-through">{b.value}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="flex flex-col justify-center">
                    <div className="text-center mb-4">
                      <p className="text-muted-foreground mb-1">De <span className="line-through">R$ 697,00</span> por apenas:</p>
                      <div className="mb-1">
                        <span className="text-5xl md:text-6xl font-black text-accent">R$ 297</span>
                        <span className="text-xl text-muted-foreground">,00</span>
                      </div>
                      <p className="text-base text-foreground font-semibold">ou <span className="text-primary font-bold">12x de R$ 30,72</span></p>
                      <p className="text-sm text-muted-foreground">no cartão de crédito</p>
                    </div>
                    <button onClick={() => (window as any).openCheckout?.()} className="w-full bg-success hover:bg-success/90 text-white font-black text-lg py-4 rounded-2xl shadow-xl shadow-success/30 hover:shadow-success/50 hover:scale-[1.02] transition-all mb-3">
                      QUERO GARANTIR MINHA VAGA
                    </button>
                    <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground mb-4">
                      <Shield className="w-4 h-4 text-accent" /><span>Compra 100% segura • Acesso imediato</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={i} className="flex items-center gap-2 text-xs text-muted-foreground bg-slate-50 rounded-lg p-2">
                          <b.icon className="w-4 h-4 text-primary" /><span>{b.text}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {/* Guarantee */}
            <div className="mt-6 bg-accent/5 rounded-2xl p-6 border-2 border-accent/20 text-center">
              <div className="flex items-center justify-center gap-4 mb-3">
                <Shield className="w-14 h-14 text-accent" />
                <div className="text-left">
                  <h3 className="text-xl font-black text-foreground">Garantia Incondicional de 7 Dias</h3>
                  <p className="text-muted-foreground text-sm">Risco ZERO para você experimentar</p>
                </div>
              </div>
              <p className="text-foreground max-w-2xl mx-auto text-sm">
                Se por qualquer motivo você não ficar satisfeito(a) com o curso nos primeiros 7 dias,
                basta enviar um e-mail e devolvemos <strong>100% do seu dinheiro</strong>. Sem perguntas, sem burocracia.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ─── FINAL CTA (Light) ─── */}
      <section className="py-10 md:py-16 bg-gradient-to-b from-slate-100 to-slate-200 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-destructive/10 border border-destructive/30 text-destructive px-4 py-2 rounded-full text-sm font-bold mb-6 animate-pulse">
              <Zap className="w-4 h-4" />ÚLTIMAS VAGAS COM DESCONTO
            </div>
            <h2 className="text-3xl md:text-5xl font-black text-foreground mb-4 leading-tight">
              Daqui a 30 dias você vai <span className="text-primary">agradecer</span> por ter começado hoje
            </h2>
            ...
            <div className="bg-white rounded-2xl p-6 mb-6 border border-slate-200 shadow-lg inline-block">
              <div className="flex items-center gap-6 flex-wrap justify-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-primary">12x R$ 30,72</p>
                </div>
              </div>
            </div>
            <div>
              <button onClick={() => (window as any).openCheckout?.()} className="group inline-flex items-center justify-center gap-3 bg-success hover:bg-success/90 text-white font-black text-xl md:text-2xl px-12 md:px-16 py-5 rounded-full shadow-2xl shadow-success/30 hover:shadow-success/50 hover:scale-105 transition-all mb-6">
      ...
      <section className="py-8 md:py-12 bg-slate-50">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-6">
              <div className="inline-flex items-center gap-2 bg-amber-100 text-amber-800 px-4 py-2 rounded-full text-sm font-semibold mb-3">
                <AlertCircle className="w-4 h-4" />Informação Importante
              </div>
              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-2">Sobre o Prazo de 30 Dias</h2>
              <p className="text-muted-foreground max-w-2xl mx-auto text-sm">Transparência é fundamental. Entenda como funciona minha estimativa de aprendizado.</p>
            </div>
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 mb-4">
              <div className="flex items-start gap-4 mb-4">
                <div className="w-12 h-12 bg-amber-100 rounded-xl flex items-center justify-center flex-shrink-0"><Clock className="w-6 h-6 text-amber-600" /></div>
                <div>
                  <h3 className="text-base font-bold text-foreground mb-1">O prazo de "30 dias" é uma estimativa</h3>
                  <p className="text-muted-foreground text-sm mb-2"><em className="text-amber-700 font-medium">*Resultados variam conforme dedicação. Estimativa baseada em 1h/dia de estudo.</em></p>
                  <p className="text-muted-foreground text-sm">
                    O período de 30 dias é uma <strong>estimativa baseada em estudos de aproximadamente 1 hora por dia</strong>.
                  </p>
                </div>
              </div>
              <div className="border-t border-slate-100 pt-4">
                <h4 className="font-semibold text-foreground mb-3 text-sm">Fatores que influenciam seu resultado:</h4>
                <div className="grid md:grid-cols-2 gap-3">
                  {[
                    { icon: BookOpen, title: "Dedicação aos estudos", desc: "Quanto mais praticar, mais rápido" },
                    { icon: Target, title: "Experiência prévia", desc: "Familiaridade inicial com tecnologia" },
                    { icon: Clock, title: "Tempo disponível", desc: "Horas dedicadas por dia" },
                    { icon: AlertCircle, title: "Ritmo individual", desc: "Cada pessoa aprende no seu tempo" },
                  ].map((item, i) => (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-xs text-amber-800">
                  ...
                  <ul className="space-y-0.5 text-amber-700">
                    <li>• O prazo de 30 dias <strong>não é uma garantia</strong> de domínio completo.</li>
                    <li>• Resultados variam conforme dedicação e ritmo de cada aluno.</li>
                    <li>• Ofereço <strong>garantia de 7 dias</strong> para reembolso integral.</li>
                  </ul>
                </div>
              </div>
            </div>
