...
export const CourseEnrollmentDialog = ({ open, onOpenChange }: CourseEnrollmentDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="w-[calc(100%-1rem)] max-w-md max-h-[96dvh] overflow-y-auto gap-0 border-primary/20 bg-background p-4 pt-10 shadow-2xl sm:rounded-xl md:p-6 md:pt-10"
        onOpenAutoFocus={(event) => event.preventDefault()}
      >
        <DialogTitle className="flex items-center justify-center gap-2 text-center text-xl font-black leading-tight text-foreground md:text-2xl">
        ...
        <div className="mt-4 text-center">
          <div className="relative mx-auto w-fit">
            <img
              ...
              className="h-28 w-28 rounded-full border-4 border-primary/20 object-cover shadow-card"
            />
            <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-success px-3 py-1 text-xs font-black text-success-foreground shadow-card">
              PROFª ELISA
            </span>
          </div>
          ...
          <div className="mt-2 flex flex-wrap items-center justify-center gap-x-2 gap-y-1">
            ...
            <span className="text-sm font-bold text-foreground">+15.000 alunos já aprenderam comigo</span>
          </div>
        </div>
